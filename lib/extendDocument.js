...
module.exports = function(mongoose, graphdb, globalOptions) {
  var Document, functionName, node, _i, _len, _queryGraphDB, _ref;

  if (globalOptions.overwriteProtypeFunctions !== true) {
  ...
  _queryGraphDB = function(cypher, options, cb) {
    ...
    if ((_ref2 = options.loadDocuments) == null) {
      options.loadDocuments = true;
    }
    if ((_ref3 = options.countRelationships) == null) {
  ...
  Document.prototype.findCorrespondingNode = function(options, cb) {
    var collectionName, doc, id, _processNode, _ref1, _ref2, _ref3;

    _ref1 = processtools.sortOptionsAndCallback(options, cb), options = _ref1.options, cb = _ref1.cb;
    doc = this;
    collectionName = doc.constructor.collection.name;
    id = processtools.getObjectIDAsString(doc);
    if ((_ref2 = options.doCreateIfNotExists) == null) {
      options.doCreateIfNotExists = false;
    }
    if ((_ref3 = options.forceCreation) == null) {
    ...
    if (doc.isNew === true && options.forceCreation !== true) {
      return cb(new Error("Can't return a 'corresponding' node of an unpersisted document"), null);
    } else if (doc._node_id) {
      return graphdb.getNodeById(doc._node_id, function(errFound, node) {
        if (errFound) {
          return cb(errFound, node);
        } else {
  ...
  Document.prototype.queryRelationships = function(typeOfRelationship, options, cb) {
    var doc, id, _ref1, _ref2, _ref3, _ref4, _ref5, _s;
    ...
    return this.getNode(function(nodeErr, fromNode) {
      ...
      if ((_ref6 = options.startNode) == null) {
  ...
  return Document.prototype.shortestPathTo = function(doc, typeOfRelationship, options, cb) {
    var from, to, _ref1;
