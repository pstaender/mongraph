...
module.exports = function(mongodb, graphdb, globalOptions) {
  var Document, node;

  Document = mongodb.Document;
  node = graphdb.createNode();
  Document.prototype._graph = {
    db: graphdb
  };
  ...
  Document.prototype.queryRelationships = function(kindOfRelationship, options, cb) {
    var cypher, doc, id, _ref, _ref1, _ref2, _ref3, _ref4, _ref5;
            ...
            if (options.loadDocuments && (map != null ? map.length : void 0) > 0) {
              data = (function() {
                ...
                for (_i = 0, _len = map.length; _i < _len; _i++) {
                  result = map[_i];
                  _results.push(result[options.processPart] || result);
                }
                return _results;
              })();
              return processtools.loadDocumentsFromRelationshipArray(doc.db, data, cb);
            } else {
              if (typeof cb === 'function') {
                return cb(err, map);
              }
