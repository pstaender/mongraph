  ...
  describe 'mongraph', ->
    ...
    describe '#init()', ->
      ...
      it 'expect to get an error if not passing mongoose + neo4j handlers as arguments', ->
        ...
        expect(-> mongraph.init({ mongoose: mongoose, neo4j: graph, mongodb: mongo })).not.to.throwError()
  ...
  describe 'Neo4j::Node', ->
    ...
    describe '#getDocument()', ->
      ...
      it 'wahtever', (done) ->
        ...
        johannes = new Person(name: 'Johannes')
        philipp = new Person(name: 'Philipp')

        johannes.save -> philipp.save ->
          philipp.createRelationshipTo johannes, 'wohnt', { seit: 'heute' }, (err, relation) ->
            # console.log err, relation
            johannes.allRelationships 'wohnt', (err, relations, found) ->
              console.log relations[0].id
              done()
