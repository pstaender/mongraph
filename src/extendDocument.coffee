...
module.exports = (mongodb, graphdb, globalOptions) ->
  ...
  Document::findEquivalentNode = (cb, doCreateIfNotExists = false) ->
    ...
    # return cb(null, doc._graph.node) if globalOptions.cacheNodes and doc._graph.node?.id
      ...
      if doCreateIfNotExists and not node
        ...
        node.save (saveErr) ->
          ...
          node.index collectionName, '_id', id, (indexErr) ->
            ...
            # doc._graph.node = node if globalOptions.cacheNodes and not indexErr or saveErr
      ...
      @getNode (nodeErr, fromNode) ->
        ...
        if options.dontExecute
          ...
          graphdb.query cypher, (err, map) ->
            # add query to error if we have one, better debugging
            ...
            if options.loadDocuments and map?.length > 0
              ...
              data = for result in map
                result[options.processPart] or result
              processtools.loadDocumentsFromRelationshipArray doc.db, data, cb
            else
